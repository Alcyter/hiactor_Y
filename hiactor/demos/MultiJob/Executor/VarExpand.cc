...
// hiactor::DataType map_expand_var(const hiactor::DataType& input, int label_index, int ID_site, int distance_site, bool is_option)
// {
...
hiactor::DataType map_expand_var(const hiactor::DataType& input, int label_index, int ID_site, int distance_site, bool is_option, std::bitset<100000>& visit)
{
    // std::cout<<"---------------Varexpand=-------\n";
    std::vector<hiactor::InternalValue> _input = *input._data.vectorValue;
    long long ID_from = _input[ID_site].intValue;
...
hiactor::DataType map_distinct(const hiactor::DataType& input, int label_index, int ID_site, int distance_site, bool is_option, std::bitset<100000>& visit)
{
    std::vector<hiactor::InternalValue> _input = *input._data.vectorValue;
    long long ID_from = _input[ID_site].intValue;
    std::vector<hiactor::InternalValue> output;
    ...
    if(visit[G -> bitset_map[ID_from]] == 1) {}
    else 
    {
        visit[G -> bitset_map[ID_from]] = 1; 
        hiactor::InternalValue result_path;
        result_path.vectorValue = new std::vector<hiactor::InternalValue>(_input);
        output.push_back(result_path);
    }
