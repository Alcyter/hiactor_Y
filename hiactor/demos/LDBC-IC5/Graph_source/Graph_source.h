...
public:
    ...
    std::bitset<16500> interested_tag;
    std::unordered_map<std::string, long long> name_to_id;

    std::unordered_map<long long, int> bitset_map_forum;
    std::unordered_map<long long, long long> post_map_forum;
...
//图构建
// <label, <ID, <property_name, Value>>>
// map<string(label), map<int(ID), map<string(property_name), InternalValue>>>

// <label, <(ID,ID), <property_name, Value>>>
// map<string(label), map<tuple<ID, ID>, map<string(property_name), InternalValue>>>

// <label, <ID, vec<ID>>>
// map<string(label), map<int(ID), vector<int(ID)>>>


//executor 传参数

// nodeByID(10),expand("know"),expand("know"),expand("know"),expand("study"),project("classyear")
