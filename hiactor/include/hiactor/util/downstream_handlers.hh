...
public:
    downstream_group_handler(hiactor::scope_builder& builder, unsigned ds_op_id, const unsigned batch_sz = batch_size)
        ...
        for (unsigned i = 0; i < hiactor::global_shard_count(); i++) {
            //hiactor中scope_builder函数名由reset_shard更改为set_shard
            builder.set_shard(i);
            _refs.emplace_back(builder.new_ref<OpRefT>(ds_op_id));
        }
        _data_buf.reserve(hiactor::global_shard_count());
        for (unsigned i = 0; i < hiactor::global_shard_count(); i++) {
...
public:
    downstream_handler(hiactor::scope_builder& builder, unsigned ds_op_id) : _have_send_data(false) {
        _ds_op = builder.new_ref<OpRefT>(ds_op_id);
    }
    ~downstream_handler() { delete _ds_op; }

    inline OpRefT* Get() { return _ds_op; }
    inline void setFunc(InDataT&&... data) {
        _ds_op->setFunc(std::forward<InDataT>(data)...);
        if (!_have_send_data) { _have_send_data = true; }
    }
    ...
    inline void receive_eos(unsigned times = 1) {
        for (unsigned i = 0; i < times; i++) {
            _ds_op->receive_eos(hiactor::Eos{IfSync | _have_send_data});
        }
    }
};
...
public:
    compressed_downstream_group_handler(hiactor::scope_builder& ds_builder, unsigned ds_op_id,
                                        const unsigned batch_sz = batch_size)
            : _cur_found_ds_num(0), _batch_sz(batch_sz), _sync_ref(nullptr) {
        _ds_refs.reserve(hiactor::global_shard_count());
        for (unsigned i = 0; i < hiactor::global_shard_count(); i++) {
            ds_builder.set_shard(i);
        ...
        _ds_flags.resize(hiactor::global_shard_count(), false);
    }
    ...
    inline void set_sync_op(hiactor::scope_builder& sync_builder, unsigned sync_op_id) {
...
public:
    ...
    inline void set_ds_ref_1(hiactor::scope_builder& ds_1_builder, unsigned ds_1_op_id) {
        if (_ds_ref_1) { delete _ds_ref_1; }
        _ds_ref_1 = ds_1_builder.new_ref<DsRefT1>(ds_1_op_id);
    }
    inline void set_ds_ref_2(hiactor::scope_builder& ds_2_builder, unsigned ds_2_op_id) {
