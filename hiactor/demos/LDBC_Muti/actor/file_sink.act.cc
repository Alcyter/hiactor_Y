...
FileSinkActor_Ref::FileSinkActor_Ref(): ::hiactor::reference_base() 
{ 
    // std::cout<<"FileSinkActor_Ref\n";
    actor_type = hiactor::TYPE_FILESINK; 
}

void FileSinkActor_Ref::process(hiactor::DataType &&input) {
	addr.set_method_actor_tid(hiactor::TYPE_FILESINK);
    // std::cout << "File_sink_Actor ";
    //     for (hiactor::byte_t elem : this -> addr.data) {
...
void FileSinkActor::output() {
    ...
    for(unsigned j = 0; j < _buf.size(); j++) {
        ...
        // 根据shard id构建文件名
        std::string file_name = "/home/yaojx/hiactor/demos/LDBC_Muti/result/shard" + std::to_string(shard_id) + ".txt";
