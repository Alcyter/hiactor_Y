...
//            case hiactor::DataType::VECTOR_TUPLE: {// input: vector<tuple*>*, output:tuple<string, int>*
//                    // _vec is vector<tuple*>
...
//
//                        unsigned dst_shard = _func.process(_vec[i]) % hiactor::global_shard_count();
//
//                        _output.type = hiactor::DataType::TUPLE;
//                        _output._data.tupleValue = new std::tuple<hiactor::InternalValue, hiactor::InternalValue>(_word_count_tuple);
//
//                        // std::cout << "Tuple " << i << ": " << std::get<0>(*_output._data.tupleValue).stringValue << ", "
//                        //                                     << std::get<1>(*_output._data.tupleValue).intValue << '\n';
//
//                        if (_have_next && _use_buffer) {
//                            emplace_data(dst_shard, std::move(_output));
//                        } else if (_have_next) {
...
//                            std::cout << "_vec size is: " << _vec.size() << '\n';
//                            std::cout << "Tuple " << i << ": " << std::get<0>(*_output._data.tupleValue).stringValue << ", "
//                                                                << std::get<1>(*_output._data.tupleValue).intValue << '\n';
//                        }
//                    }
