...
class MapActor : public hiactor::actor {
public:
    MapActor(hiactor::actor_base* exec_ctx, const hiactor::byte_t* addr) : hiactor::actor(exec_ctx, addr, false) {
...
class MapActor_Ref : public ::hiactor::reference_base, public hiactor::Actor_Ref {
public:
    MapActor_Ref();
    ~MapActor_Ref() override = default;
    /// actor methods
    void setNext(hiactor::actor_config_Batch&& next);
    void setFunc(hiactor::MapFunc &&func);
    void process(hiactor::DataType &&input);
...
class Map : public hiactor::OperatorBase {
public:
    Map(hiactor::scope_builder& builder, unsigned ds_op_id, const unsigned batch_sz = batch_size)
