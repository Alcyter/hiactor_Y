...
void transferToOneHop(long long id, int relation_expand) {
    if(G -> interested_tag[id] == 1) {
        return;
    }
    G -> interested_tag[id] = 1;
    for(long long nei : G -> neighbor[relation_expand][id]) {
        transferToOneHop(nei, relation_expand);
    }
}
...
hiactor::DataType transfer(const hiactor::DataType& input, std::string name, int relation_name, int relation_expand) {
    ...
    transferToOneHop(target_id, relation_expand);
