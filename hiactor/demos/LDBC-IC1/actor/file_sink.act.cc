...
void FileSinkActor::output() {
    ...
    for(unsigned j = 0; j < _buf.size(); j++) {
        ...
        // 根据shard id构建文件名
        std::string file_name = "/home/yaojx/hiactor/demos/LDBC-IC1/result/shard" + std::to_string(shard_id) + ".txt";
...
//                    //std::cout<<_schema[n].stringValue<< " ";
//                } else {
//                    out_file << _schema[n].intValue;
//                    //std::cout<<_schema[n].intValue<< " ";
//                }
//                out_file <<" ";
                if(n == 0 || n == 1) {
                    out_file << _schema[n].intValue;
