...
//     hiactor::InternalValue node;
//     std::vector<hiactor::InternalValue> ori_path = *input._data.vectorValue;
//     hiactor::InternalValue result_path;
//     long long ID_from = ori_path[ID_site].intValue;
//     // std::vector<hiactor::InternalValue> output;
...
//         (*Data._data.vectorValue).push_back(result_path);
...
hiactor::DataType map_expand_vec(const hiactor::DataType& input, int label_index, int ID_site, int distance_site, bool is_option, hiactor::InternalValue& node, std::vector<hiactor::InternalValue>& ori_path )
{
    ...
    // hiactor::InternalValue node;
    // std::vector<hiactor::InternalValue> ori_path = *input._data.vectorValue;
    ori_path = *input._data.vectorValue;
    ...
    if(ID_from == -1) { //then add NULL to the path
        ...
        node.vectorValue = new std::vector<hiactor::InternalValue>(ori_path);        
        (*Data._data.vectorValue).push_back(node);
