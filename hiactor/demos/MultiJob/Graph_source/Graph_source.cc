...
//     // std::cout << "what??\n";
//     std::string path="/home/yaojx/hiactor/demos/LDBC-IC5/Graph_source";
...
//                         if (kk == 1)
//                         {
//                             bitset_map_forum[ID] = index_forum;
//                             index_forum++;
//                         }
//                     }
//                     else {
...
//                     }                    
//                 }
//                 if (kk == 5) //post_map_forum
//                 {
//                     post_map_forum[ID2]=ID1;
//                 }
//                 if (kk>=23)  //建反向边
//                 {
...
void Graph_source::Get_Node_data() {
    ...
    fname = path + "/static/tagclass_0_0.csv";
    vec_fname.push_back(fname);
    fname = path + "/dynamic/message_0_0.csv";
    vec_fname.push_back(fname);
            ...
            while (getline(csv_data, line)) //每行处理
            {
                ...
                for (unsigned i = 0; i < properties_name.size(); i++) {
                    ...
                    if (i == 0) {
                        ...
                        if((kk == 0 && i == 1) || (kk == 0 && i == 5) || (kk == 1 && i == 2) || (kk == 2 && i == 4) || (kk == 2 && i == 5) || (kk == 3 && i == 2)
                            || (kk == 3 && i == 7) || (kk == 8 && i == 2) || (kk == 8 && i == 7)) {
                            _str.intValue = atoll(word.c_str());
                        } else {
                            _str.stringValue = new char[word.length() + 1];
                            strcpy(_str.stringValue, word.c_str());
                        }
...
void Graph_source::Get_Edge_data()
{
    ...
    fname= path + "/dynamic/forum_containerOf_post_0_0.csv";
    vec_fname.push_back(fname);
    fname= path + "/dynamic/forum_hasMember_person_0_0.csv"; // 6 2
    vec_fname.push_back(fname);
    fname= path + "/dynamic/forum_hasModerator_person_0_0.csv";
    vec_fname.push_back(fname);
    fname= path + "/dynamic/forum_hasTag_tag_0_0.csv";
    ...
    fname= path + "/dynamic/person_isLocatedIn_place_0_0.csv";
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_knows_person_0_0.csv"; // 11 2
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_likes_comment_0_0.csv"; // 12 2
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_likes_post_0_0.csv"; // 13 2
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_studyAt_organisation_0_0.csv"; // 14 2
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_workAt_organisation_0_0.csv"; // 15 2
    vec_fname.push_back(fname);
    ...
    fname= path + "/dynamic/message_isLocatedIn_place_0_0.csv";
    vec_fname.push_back(fname);
    ...
    fname= path + "/dynamic/comment_hasCreator_person_0_0.csv";
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_likes_post_0_0.csv"; // 26 2
    vec_fname.push_back(fname);
    fname= path + "/dynamic/comment_replyOf_post_0_0.csv";
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_hasInterest_tag_0_0.csv";
    ...
    fname= path + "/dynamic/message_hasCreator_person_0_0.csv";
    vec_fname.push_back(fname);
    fname= path + "/dynamic/person_likes_message_0_0.csv"; // 31 2 intValue
    vec_fname.push_back(fname);
    fname= path + "/dynamic/comment_replyOf_message_0_0.csv";
    vec_fname.push_back(fname);
    ...
    for (unsigned kk=0;kk<vec_fname.size();kk++)
    {
        ...
        else
        {
            ...
            sin.str(line);
            if(kk == 31) {
                properties_name.emplace_back("ID1");
                properties_name.emplace_back("ID2");
                properties_name.emplace_back("creationDate");
            } else {
                while (getline(sin,word,'|'))
                {
                    properties_name.push_back(word);
            ...
            while (getline(csv_data, line))
            {
                ...
                for (unsigned i=0;i<properties_name.size();i++)
                {
                    ...
                    {
                        if((kk == 6 && i == 2) || (kk == 11 && i == 2) || (kk == 12 && i == 2) || (kk == 13 && i == 2) || (kk == 14 && i == 2) || (kk == 15 && i == 2) || (kk == 26 && i == 2) || (kk == 31 && i == 2) || (kk == 33 && i == 2)) {
                            _str.intValue = atoll(word.c_str());
                        } else {
                            _str.stringValue= new char[word.length()+1];
                            strcpy(_str.stringValue,word.c_str());
